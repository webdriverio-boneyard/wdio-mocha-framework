...
adapter.wrapHook = function (config, specs, hookFn, runner, type) {
    // Hooks with an instance of Mocha runner can access current test information, but
    // need to call done() to notify async completion. Hooks outside Mocha context (e.g.
    // before/after) are simpler.
    if (runner) {
