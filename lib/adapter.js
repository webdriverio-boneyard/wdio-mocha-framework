...
class MochaAdapter {
    ...
    formatMessage (params) {
        ...
        if (params.payload) {
            // Add the current test title to the main title if it adds more value, e.g.
            // when running inside a beforeEach hook
            if (params.payload.ctx.currentTest && (params.payload.ctx.currentTest.title !== params.payload.title)) {
                message.title = `${params.payload.title} for "${params.payload.ctx.currentTest.title}"`
            } else {
