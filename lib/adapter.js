...
adapter.run = async function mochaAdapter (cid, config = { mochaOpts: {} }, specs = [], capabilities) {
    ...
    let result = await new Promise((resolve, reject) => {
        ...
        runner.suite.beforeAll(hookWrapper(config.beforeSuite, runner, true))
        runner.suite.beforeEach(hookWrapper(config.beforeTest, runner, true))
        runner.suite.afterEach(hookWrapper(config.afterTest, runner, true))
        runner.suite.afterAll(hookWrapper(config.afterSuite, runner, true))
    })
    await hookWrapper(config.after)
    return result
}

adapter.wrapHook = function (config, specs, hookFn, runner, async) {
    if (async) {
        return function (done) {
            let message = adapter.formatMessage({
                payload: runner.test || runner.suite.suites[0],
